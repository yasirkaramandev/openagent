...
class TestDoctorV2Checks:
    ...
    def test_the_registered_v2_migrations_are_reported_as_ok(self) -> None:
        """Was a WARN while 0015-0017 were held back for 0014. 0014 shipped in 0.1.6."""

        from openagent.storage.migrations_v2 import registration_status

        check = migration_hold_check(registration_status())
        assert check.status == OK
        assert "0015" in check.detail

    def test_a_held_back_set_is_still_reported_as_a_warning(self) -> None:
        """The WARN branch is kept alive: it is what a future held fragment would report."""

        check = migration_hold_check(
            {
                "registered": False,
                "revisions": ["0018"],
                "base_revision": "0017",
                "base_revision_present": False,
                "reason": "revision 0017 is not in this build's migration chain",
            }
        )
        assert check.status == WARN
        assert "0017" in check.detail
