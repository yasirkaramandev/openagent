...
def test_provider_roundtrip(repos: Repositories):
    provider = ProviderConnection(
        id="provider_deepseek_main", name="deepseek-main", provider_type="deepseek",
        protocol=Protocol.OPENAI_CHAT, base_url="https://api.deepseek.com",
    )
...
def test_agent_roundtrip(repos: Repositories):
    agent = AgentProfile(
        name="deepseek-coder", title="DeepSeek Coder",
        runtime=AgentRuntime(type=RuntimeType.API_AGENT, provider="deepseek-main", model="m"),
        tags=["coder", "python"], permission_profile="safe-edit",
    )
...
def test_event_log_writes_and_indexes(tmp_path: Path, repos: Repositories):
    ...
    log.append(NormalizedEvent(run_id="run_01ABC", type=EventType.RUN_COMPLETED, source="openagent"))
...
def test_event_log_redacts_secrets(tmp_path: Path):
    ...
    log.append(
        NormalizedEvent(
            run_id="run_x", type=EventType.LOG, source="api-agent",
